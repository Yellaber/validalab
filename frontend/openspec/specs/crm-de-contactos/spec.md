# crm-de-contactos Specification

## Purpose
CRM de contactos y embudo de outreach en el cliente Angular (épica E3): listar paginado con filtro por estado, crear, consultar, editar y eliminar los contactos de una idea propia, moverlos por el embudo `por_contactar → contactado → respondio → agendado → entrevistado → descartado` y registrar hasta dos toques por contacto, incluyendo el vínculo de referido dentro de la misma idea. Consume el tag `contactos` del contrato de API único. El estado `entrevistado` solo lo origina el registro de una entrevista (E4) y no es un destino de transición manual; los KPIs de alcance que se derivan de este embudo (E5) y el veredicto (E6) quedan fuera. Un `Contacto` es información personal: ninguna vista revela dato alguno de un contacto ajeno. Se apoya en la plomería del E0 (`cliente-http-y-errores`, `sesion-cliente`, `shell-y-navegacion`) y en `portafolio-de-ideas` (E1).
