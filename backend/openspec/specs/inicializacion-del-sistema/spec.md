# inicializacion-del-sistema Specification

## Purpose

Arranque operativo del backend: el endpoint `POST /sistema/inicializar` que crea la cuenta administradora de origen de una instalación. Es el **único** camino por el que existe el rol `administrador` —el registro público asigna siempre `validador` y cambiar el rol exige ya serlo—, de modo que sin esta capacidad las operaciones de administración de cuentas serían inalcanzables por API y solo accesibles con un `UPDATE` manual sobre la base de datos.

Cubre tres responsabilidades que se sostienen entre sí: la protección por un secreto de despliegue (que cierra la ventana en la que un tercero podría reclamar el administrador de un sistema recién expuesto), el marcador persistido que vuelve la operación de un solo uso y arbitra en la base de datos la carrera entre llamadas simultáneas, y la reutilización del alta de cuentas de `usuarios` para no duplicar hashing ni invariantes. El comportamiento observable lo fija la capacidad `sistema` del contrato (`openspec/specs/sistema/` en la raíz); esta capacidad describe cómo lo cumple el runtime.
