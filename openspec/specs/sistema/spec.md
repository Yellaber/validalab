...
Ciclo de vida de la **instalación** de ValidaLab, no de su dominio: las operaciones que se ejecutan al poner en marcha un sistema y no forman parte del uso diario. Hoy cubre la inicialización de un sistema virgen —de un solo uso, protegida por un secreto de despliegue— que produce la primera y única cuenta administradora de origen, sin la cual las operaciones de administración de cuentas serían inalcanzables por API. Esta capacidad se expresa como el detalle del `tag` `sistema` en el contrato de API único (`contrato-api/openapi.yaml`); es el único `tag` que no corresponde a un módulo de dominio, y se acota deliberadamente a ese propósito para que no degenere en cajón de sastre. Su cumplimiento en runtime (módulo `sistema`, marcador de inicialización persistido, validación del secreto al arranque) se aborda en cambios posteriores.
