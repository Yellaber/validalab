...
El contrato de API es la fuente de verdad única que dirige el desarrollo de `frontend/` y `backend/` en ValidaLab, de modo que ambos equipos trabajen contra una misma definición sin conocer ni depender del código del otro. Se materializa como un único documento OpenAPI canónico, navegable por `tags` de dominio; `CLAUDE.md` retiene solo el índice y el resumen normativo de las convenciones transversales, manteniendo la memoria acotada sin perder la consulta integral.
